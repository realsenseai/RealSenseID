...
#USB\VID_414C&PID_6578&MI_00\6&1C727CC6&0&0000
...
$MultiPinDevices =
    "USB\VID_414C&PID_6578&MI_00"# F500
...
#Filter Realsense ID devices
$ConnectedDev = $ConnectedDev -like "*VID_414C*"

#Progress notification
$rs_count = $ConnectedDev.Count
echo "$rs_count connected RealSense devices were found:" $ConnectedDev

#Search each subtree for values that correspond to the requested Realsense ID devices
foreach ($subtree in $SearchTrees)
{
    ...
    $Items = $Items | Where { $_.DeviceInstance -like "*VID_414C*" }
    "" + $Items.Count + " of them are Realsense ID"
